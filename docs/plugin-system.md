...
`enabled` 是內建的通用慣例：engine 會在 `getPromptVariables()` 與 `getDynamicVariables()` 中中央化跳過 disabled plugin 的提示詞與動態變數，並在 action-button API 過濾按鈕。Plugin 自身仍必須在前端 hook 與後端 hook 中讀取設定並自行 no-op。`promptStripTags` 與 `displayStripTags` 不受 `enabled` 影響，這是刻意設計，確保 plugin 停用後歷史章節中的標籤仍能被清除。
