...
| `action-button:click` | 使用者點擊 `PluginActionBar` 中由 plugin 貢獻的按鈕時觸發；async dispatch 並依 `originPluginName` 過濾 | `{ buttonId, pluginName, series, name, storyDir, lastChapterIndex, runPluginPrompt, notify, reload }`，詳見「[動作按鈕（Action Buttons）](action-buttons.md)」 |
