...
| `insert-transform` | `insert: true` plugin-action 串流結束後、解析插入信封**之前**（序列、**可變**、**未** deep-frozen） | `{ correlationId, pluginName, rawResponse, numberedParagraphs, series, name, storyDir, envelope }` — 唯一可寫欄位為 `envelope: string \| null`，詳見 [insert-transform：插入信封轉換](#insert-transform插入信封轉換) |
...
### `insert-transform`：插入信封轉換

`insert-transform` 是專為 `runPluginPrompt(..., { insert: true })` 插入寫入模式設計的後端 hook。它在 plugin-action 串流**完整累積完成之後**、引擎**解析插入信封之前**分派，讓 plugin 可以把 LLM 的自訂回應格式（例如一個扁平的 per-image JSON 陣列）轉換成引擎能理解的標準插入信封 `{ "insertions": [ { "insertAfterParagraph": <int>, "text": <string> } ] }`。

與其他多數後端 stage 不同，`insert-transform` 有以下特性：

- **序列、可變、未 deep-frozen**：context 物件**不會**被凍結，handler **可以**寫入 `envelope` 欄位（唯一可寫欄位）。它**不在** `PARALLEL_ALLOWED` 白名單內，永遠序列執行。
- **Origin 自我過濾（強制）**：dispatcher 不會自動依 plugin 過濾此 stage，因此 handler **必須**自行檢查 `context.pluginName === "<自己的 plugin 名稱>"`，否則直接 `return`，避免一個 plugin 轉換到另一個 plugin 的插入回應。
- **回退到原始回應**：當沒有任何 handler 設定 `envelope`（維持初始 `null`）時，引擎會直接解析 `rawResponse`。若該原始回應不是合法的插入信封，引擎會以 `plugin-action:invalid-insert-payload` 失敗，且章節**不被更動**。
- **例外被吞掉**：序列後端 hook 拋出的例外會被 dispatcher 吞掉並記錄。因此 handler 在驗證失敗時**不可依賴 `throw`**——應改為「不設定 `envelope` 並 `return`」，讓引擎走回退路徑後以 `invalid-insert-payload` 收場。
- **不得做檔案 I/O**：handler 只負責產生信封字串，章節的讀寫與 splice 由引擎在 per-story generation lock 內完成。

Context 欄位（型別見 `writer/types/hooks.ts` 的 `InsertTransformPayload`）：

| 欄位 | 型別 | 說明 |
|------|------|------|
| `correlationId` | `readonly string` | 與同次 plugin-action 插入請求一致 |
| `pluginName` | `readonly string` | 發起插入的 plugin 名稱（origin 過濾鍵） |
| `rawResponse` | `readonly string` | 完整累積的 LLM 回應，原樣未正規化 |
| `numberedParagraphs` | `readonly string` | LLM 看到的 `numbered_paragraphs` 渲染字串 |
| `series` | `readonly string` | `playground/` 下的系列名稱 |
| `name` | `readonly string` | `playground/<series>/` 下的故事名稱 |
| `storyDir` | `readonly string` | 故事目錄的絕對路徑 |
| `envelope` | `string \| null` | **唯一可寫的輸出欄位**。初始為 `null`；handler 寫入標準插入信封 JSON 字串以讓引擎改為解析它 |

範例（origin 過濾 + 設定 `envelope`）：

```javascript
hooks.register("insert-transform", (context) => {
  // Origin self-filter: only transform our own plugin's insert runs.
  if (context.pluginName !== "my-plugin") return;
  const envelope = buildEnvelopeFromRaw(context.rawResponse); // string | null
  // On any validation failure, leave envelope unset and return — do NOT throw.
