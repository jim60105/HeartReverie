    ...
    try {
      ...
      await t.step("PUT creates a new global passage → 201", async () => {
        const res = await makeRequest(
          app,
          "PUT",
          "/api/lore/global/hero.md",
          passageBody(["characters"], "The hero is brave."),
        );
      ...
      await t.step("PUT updates an existing global passage → 200", async () => {
        const res = await makeRequest(
          ...
          "PUT",
          "/api/lore/global/hero.md",
          passageBody(["characters"], "The hero is very brave.", 5),
        );
      ...
      await t.step(
        "GET reads the created global passage with correct frontmatter and content",
        async () => {
      ...
      await t.step("PUT creates a series passage → 201", async () => {
        const res = await makeRequest(
          app,
          "PUT",
          "/api/lore/series/mySeries/world.md",
          passageBody(["worldbuilding"], "The world is vast."),
        );
        assertEquals(res.status, 201);
        assertEquals(res.body.message, "Passage created");
      });
      ...
      await t.step("PUT creates a story passage → 201", async () => {
        const res = await makeRequest(
          app,
          "PUT",
          "/api/lore/story/mySeries/myStory/npc.md",
          passageBody(["characters"], "An NPC."),
        );
        assertEquals(res.status, 201);
        assertEquals(res.body.message, "Passage created");
      });
      ...
      await t.step("GET with ?tag= filter returns only matching passages", async () => {
        ...
        await makeRequest(
          app,
          "PUT",
          "/api/lore/global/alpha.md",
          passageBody(["lore"], "Alpha content."),
        );
        await makeRequest(
          app,
          "PUT",
          "/api/lore/global/beta.md",
          passageBody(["history"], "Beta content."),
        );
      ...
      await t.step("GET /api/lore/tags aggregates tags across all scopes", async () => {
        await makeRequest(
          app,
          "PUT",
          "/api/lore/global/g1.md",
          passageBody(["magic"], "Global magic."),
        );
        await makeRequest(
          app,
          "PUT",
          "/api/lore/series/s1/s1p.md",
          passageBody(["technology"], "Series tech."),
        );
        await makeRequest(
          app,
          "PUT",
          "/api/lore/story/s1/t1/sp.md",
          passageBody(["geography"], "Story geo."),
        );
      ...
      await t.step(
        "GET /api/lore/tags skips reserved platform directories during traversal",
        async () => {
          ...
          await Deno.writeTextFile(
            join(tmpDir, "System Volume Information", "_lore", "x.md"),
            reservedPassage,
          );
          await Deno.mkdir(join(tmpDir, ".Spotlight-V100", "_lore"), { recursive: true });
          await Deno.writeTextFile(
            join(tmpDir, ".Spotlight-V100", "_lore", "x.md"),
            reservedPassage,
          );
          ...
          await Deno.mkdir(join(tmpDir, "normalSeries", "lost+found", "_lore"), {
            recursive: true,
          });
          await Deno.writeTextFile(
            join(tmpDir, "normalSeries", "lost+found", "_lore", "x.md"),
            reservedPassage,
          );
          await Deno.mkdir(join(tmpDir, "normalSeries", "$RECYCLE.BIN", "_lore"), {
            recursive: true,
          });
          await Deno.writeTextFile(
            join(tmpDir, "normalSeries", "$RECYCLE.BIN", "_lore", "x.md"),
            reservedPassage,
          );
          ...
          await Deno.mkdir(join(tmpDir, "normalSeries", ".Spotlight-V100", "_lore"), {
            recursive: true,
          });
          await Deno.writeTextFile(
          ...
          await Deno.mkdir(join(tmpDir, "normalSeries", ".fseventsd", "_lore"), {
            recursive: true,
          });
          await Deno.writeTextFile(
      ...
      await t.step("PUT creates a passage in a subdirectory", async () => {
        const res = await makeRequest(
          app,
          "PUT",
          "/api/lore/global/characters/villain.md",
          passageBody(["evil"], "A villain."),
        );
        assertEquals(res.status, 201);
      });
      ...
      await t.step(
        "GET list includes subdirectory passages with directory-implicit tag",
        async () => {
          const res = await makeRequest(app, "GET", "/api/lore/global");
          assertEquals(res.status, 200);
          const villain = res.body.find((p: Record<string, unknown>) =>
            p.filename === "villain.md"
          );
      ...
      await t.step("PUT with path traversal attempt → 400", async () => {
        const res = await makeRequest(
          app,
          "PUT",
          "/api/lore/global/sub%2F..%2Fescape.md",
          passageBody([], "content"),
        );
        assertEquals(res.status, 400);
      });
      ...
      await t.step("PUT with missing frontmatter → 400", async () => {
        const res = await makeRequest(app, "PUT", "/api/lore/global/test.md", {
      ...
      await t.step("PUT with invalid series param → 400", async () => {
        const res = await makeRequest(
          app,
          "PUT",
          "/api/lore/series/bad..param/test.md",
          passageBody([], "content"),
        );
        assertEquals(res.status, 400);
      });
      ...
      await t.step("PUT with invalid story param → 400", async () => {
        const res = await makeRequest(
          app,
          "PUT",
          "/api/lore/story/ok/bad..param/test.md",
          passageBody([], "content"),
        );
        assertEquals(res.status, 400);
      });
      ...
      await t.step("PUT with enabled=false stores correctly and reflects in list", async () => {
        const res = await makeRequest(
          app,
          "PUT",
          "/api/lore/global/disabled.md",
          passageBody(["test"], "Disabled passage", 0, false),
        );
        assertEquals(res.status, 201);
        ...
        const found = listRes.body.find((p: Record<string, unknown>) =>
          p.filename === "disabled.md"
        );
