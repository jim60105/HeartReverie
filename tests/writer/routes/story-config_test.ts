  ...
  fn: async (t) => {
    ...
    try {
      await t.step("GET without auth → 401", async () => {
        const res = await makeRequest(app, "GET", "/api/series1/story1/config", undefined, {
          "x-passphrase": "",
        });
      ...
      await t.step("PUT returns 404 when story directory missing", async () => {
        const res = await makeRequest(app, "PUT", "/api/series1/ghost-story/config", {
          temperature: 0.5,
        });
      ...
      await t.step(
        "WHEN story path exists as file THEN GET returns 404 story not found",
        async () => {
      ...
      await t.step(
        "WHEN story path exists as file THEN PUT returns 404 story not found",
        async () => {
          const res = await makeRequest(app, "PUT", "/api/series1/not-a-dir/config", {
            temperature: 0.3,
          });
          assertEquals(res.status, 404);
        },
      ...
      await t.step(
        "WHEN PUT body is invalid JSON THEN returns 400 with bad request problem",
        async () => {
      ...
      await t.step("PUT rejects path traversal → 400", async () => {
        const res = await makeRequest(app, "PUT", "/api/..%2fescape/name/config", {
          temperature: 0.5,
        });
      ...
      await t.step(
        "WHEN _config.json contains malformed JSON THEN GET returns 422 validation error",
        async () => {
      ...
      await t.step(
        "WHEN Deno.stat throws non-NotFound on GET THEN returns 500 internal error",
        async () => {
      ...
      await t.step(
        "WHEN Deno.stat throws non-NotFound on PUT THEN returns 500 internal error",
        async () => {
          const statStub = stub(Deno, "stat", () => {
            throw new Error("stat failed unexpectedly");
          });
      ...
      await t.step(
        "WHEN writeStoryLlmConfig throws generic error THEN PUT returns 500 internal error",
        async () => {
      ...
      await t.step(
        "WHEN readStoryLlmConfig throws generic error THEN GET returns 500 internal error",
        async () => {
      ...
      await t.step("PUT rejects non-boolean reasoningEnabled → 400", async () => {
        ...
        assertEquals(
          typeof res.body?.detail === "string" &&
            res.body.detail.includes("reasoningEnabled"),
          true,
        );
      });
      ...
      await t.step("PUT rejects unknown reasoningEffort → 400", async () => {
        ...
        assertEquals(
          typeof res.body?.detail === "string" &&
            res.body.detail.includes("reasoningEffort"),
          true,
        );
      });
