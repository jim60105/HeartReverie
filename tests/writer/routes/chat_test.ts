...
Deno.test({
  name: "chat routes – extended coverage",
  sanitizeOps: false,
  sanitizeResources: false,
  fn: async (t) => {
    ...
    function makeDeps(overrides: Record<string, unknown> = {}): AppDeps {
      ...
      registerUserMessage({
        hooks: hd,
        logger: createLogger("plugin", { baseData: { plugin: "user-message" } }),
      });
      return {
        config: {
          READER_DIR: "/nonexistent-reader",
    ...
    await t.step("returns 400 when message exceeds max length", async () => {
      ...
      try {
        ...
        const res = await makeRequest(app, "POST", "/api/stories/s1/n1/chat", {
          message: longMessage,
        });
        assertEquals(res.status, 400);
    ...
    await t.step(
      "returns 422 with multi-message:no-user-message variant when template emits no user role",
      async () => {
    ...
    await t.step(
      "returns 422 with multi-message:empty-message variant when template emits whitespace-only message",
      async () => {
            ...
            new ReadableStream({
              pull(controller) {
                ...
                if (!chunkSent) {
                  chunkSent = true;
                  controller.enqueue(
                    encoder.encode('data: {"choices":[{"delta":{"content":"partial data"}}]}\n\n'),
                  );
                } else {
    ...
    await t.step(
      "error response surfaces upstream body — 502 includes upstream detail",
      async () => {
