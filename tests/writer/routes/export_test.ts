...
Deno.test({
  ...
  fn: async (t) => {
    ...
    await t.step("Content-Disposition encodes non-ASCII names with RFC 5987", async () => {
      ...
      const res = await app.fetch(
        new Request(
          `http://localhost/api/stories/${encodeURIComponent("系列")}/${
            encodeURIComponent("故事")
          }/export?format=md`,
      ...
      assert(
        disp.includes("%E7%B3%BB") || disp.includes("%E6%95%85"),
        `expected percent-encoded UTF-8 in: ${disp}`,
      );
