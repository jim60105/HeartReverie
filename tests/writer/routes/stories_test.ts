...
Deno.test({
  ...
  fn: async (t) => {
    ...
    try {
      ...
      await t.step(
        "GET /api/stories/:series lists subdirectories excluding underscore-prefixed",
        async () => {
      ...
      await t.step(
        "GET /api/stories/:series rejects reserved platform directory names",
        async () => {
      ...
      await t.step(
        "WHEN readDir throws non-NotFound on GET /api/stories/:series THEN returns 500",
        async () => {
          const readDirStub = stub(Deno, "readDir", () => {
