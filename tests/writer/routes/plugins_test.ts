...
Deno.test({
  ...
  fn: async (t) => {
    ...
    await t.step(
      "GET /api/plugins/action-buttons filters disabled plugins",
      async () => {
...
Deno.test({
  name: "plugin frontend module routes",
  sanitizeOps: false,
  sanitizeResources: false,
  fn: async (t) => {
    Deno.env.set("PASSPHRASE", "test-pass");
    ...
    await t.step(
      "hasFrontendModule is true when frontendModule is set",
      async () => {
        const app = createApp({
          config: {
            READER_DIR: "/nonexistent-reader",
    ...
    await t.step(
      "GET /plugins/:name/:path serves frontend module",
      async () => {
        const app = createApp({
          config: {
            READER_DIR: "/nonexistent-reader",
    ...
    await t.step(
      "GET /plugins/:name/:path returns 404 when file missing",
      async () => {
      ...
      verifyPassphrase,
        ...
        const res = await makeRequest(
          app,
          "GET",
          "/plugins/missing-plugin/nonexistent.js",
        );
    ...
    await t.step("frontendModule path escape is skipped", async () => {
      ...
      const res = await makeRequest(
        app,
        "GET",
        "/plugins/escape-plugin/escape.js",
      );
      assertEquals(res.status, 404);
    });
...
Deno.test({
  name: "shared plugin utils routes",
  sanitizeOps: false,
  sanitizeResources: false,
  fn: async (t) => {
    Deno.env.set("PASSPHRASE", "test-pass");
    ...
    await t.step(
      "GET /plugins/_shared/utils.js serves JS file with correct content-type",
      async () => {
        const app = makeApp();
        const res = await app.fetch(
    ...
    await t.step(
      "GET /plugins/_shared/secret.env returns 404 (non-JS rejected)",
      async () => {
        const app = makeApp();
        const res = await makeRequest(
          app,
          "GET",
          "/plugins/_shared/secret.env",
        );
        assertEquals(res.status, 404);
      },
    );
    ...
    await t.step(
      "GET /plugins/_shared/../../.env is rejected (path traversal)",
      async () => {
        const app = makeApp();
        const res = await makeRequest(
          app,
    ...
    await t.step(
      "GET /plugins/_shared/.secret.js returns 404 (dotfile rejected)",
      async () => {
...
Deno.test({
  name: "parameters endpoint with lore discovery",
  sanitizeOps: false,
  sanitizeResources: false,
  fn: async (t) => {
    Deno.env.set("PASSPHRASE", "test-pass");
    ...
    await t.step("without series param returns base params only", async () => {
      ...
      assert(Array.isArray(res.body));
      assert(
        !res.body.some((p: Record<string, unknown>) => p.source === "lore"),
      );
    });
    ...
    await t.step("with series param returns base + lore params", async () => {
      const res = await makeRequest(
        app,
        "GET",
        "/api/plugins/parameters?series=test",
      );
      assertEquals(res.status, 200);
      assert(Array.isArray(res.body));
      assert(
      ...
      assert(
        res.body.some((p: Record<string, unknown>) =>
          p.name === "lore_tags" && p.source === "lore"
        ),
      );
      assert(
        res.body.some((p: Record<string, unknown>) =>
          p.name === "lore_setting" && p.source === "lore"
        ),
      );
    });

    await t.step(
      "lore_all and lore_tags present even with no passages",
      async () => {
        const res = await makeRequest(
          app,
          "GET",
          "/api/plugins/parameters?series=nonexistent",
        );
        assertEquals(res.status, 200);
        assert(
          res.body.some((p: Record<string, unknown>) => p.name === "lore_all"),
        );
        assert(
          res.body.some((p: Record<string, unknown>) => p.name === "lore_tags"),
        );
      },
    );
...
Deno.test({
  name: "plugin frontendStyles routes",
  sanitizeOps: false,
  sanitizeResources: false,
  fn: async (t) => {
    Deno.env.set("PASSPHRASE", "test-pass");
    ...
    await Deno.mkdir(pluginDir, { recursive: true });
    await Deno.writeTextFile(
      join(pluginDir, "style.css"),
      ".foo { color: red; }",
    );
    await Deno.writeTextFile(
      join(pluginDir, "secret.css"),
      ".secret { color: black; }",
    );

    function makeApp(
      frontendStyles: string[],
      validatedStyles: string[] = frontendStyles,
    ) {
      return createApp({
        config: {
          READER_DIR: "/nonexistent-reader",
    ...
    await t.step(
      "GET /api/plugins includes frontendStyles URL paths",
      async () => {
        const app = makeApp(["style.css"]);
        const res = await makeRequest(app, "GET", "/api/plugins");
        assertEquals(res.status, 200);
        assertEquals(res.body[0].frontendStyles, [
          "/plugins/styled-plugin/style.css",
        ]);
      },
    );

    await t.step(
      "declared CSS file is served with text/css content type",
      async () => {
        const app = makeApp(["style.css"]);
        const res = await app.fetch(
    ...
    await t.step(
      "undeclared CSS file returns 404 even if it exists on disk",
      async () => {
        const app = makeApp(["style.css"], ["style.css"]);
        const res = await makeRequest(
          app,
          "GET",
          "/plugins/styled-plugin/secret.css",
        );
        assertEquals(res.status, 404);
      },
    );
    ...
    await t.step("path traversal attempt returns 404", async () => {
      ...
      const res = await makeRequest(
        app,
        "GET",
        "/plugins/styled-plugin/../../etc/passwd",
      );
      assert(
        res.status === 403 || res.status === 404,
        `Expected 403 or 404, got ${res.status}`,
    ...
    await t.step(
      "plugin with no frontendStyles yields empty array in /api/plugins",
      async () => {
        const app = makeApp([], []);
        const res = await makeRequest(app, "GET", "/api/plugins");
        assertEquals(res.status, 200);
