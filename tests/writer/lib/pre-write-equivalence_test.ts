...
Deno.test("6.5 Pre-write equivalence integration fixture", async (t) => {
  ...
  await t.step("serial pre-write mutation survives when parallel handlers exist on another stage", async () => {
    await withSilencedConsole(async () => {
      const hd = new HookDispatcher();
  ...
  await t.step("multiple serial handlers on pre-write compose in priority order", async () => {
    await withSilencedConsole(async () => {
      ...
      hd.register("post-response", async () => {
        await new Promise((r) => setTimeout(r, 5));
      }, { parallel: true, readOnly: true }, "metrics-plugin");
  ...
  await t.step("pre-write with empty parallel bucket is byte-identical to legacy path", async () => {
    await withSilencedConsole(async () => {
      const hd = new HookDispatcher();
