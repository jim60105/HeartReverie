...
- **THEN** `previous_context` SHALL contain: an optional global summary element (wrapped in `<story_summary>` tags), summary strings for older chapters (wrapped in `<chapter_summary>` tags), and full stripped text for recent chapters within the L2 window — all ordered numerically
