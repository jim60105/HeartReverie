...
Template errors SHALL be surfaced to the user in the chat UI as a visually distinct error card. The error card SHALL display the error message, the template file path, the line number (if available), the problematic variable (if applicable), and the fix suggestion. The error card SHALL be styled differently from normal chat messages (e.g., red/warning border, error icon) to be immediately recognizable. The error card SHALL NOT silently swallow the error or show a generic "something went wrong" message.

#### Scenario: Error card displayed in chat
- **WHEN** the chat endpoint returns an HTTP 422 template error response
- **THEN** the frontend SHALL render a visually distinct error card in the chat area showing the error details

#### Scenario: Error card shows actionable information
- **WHEN** the error card is displayed with a missing variable error
- **THEN** the card SHALL show the variable name, the template file, and the suggestion text so the user can take corrective action
