# Vento Prompt Template — Plugin System Delta
...
- `previous_context` (array of strings): Stripped chapter contents in numerical order. Each element has already been processed to remove tags registered in the `frontend-strip` hook stage (e.g., `<options>`, `<disclaimer>`, `<user_message>`).
