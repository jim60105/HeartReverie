...
- **THEN** the textarea, resend button SHALL be disabled via `:disabled="isLoading"`, and the Send button SHALL be replaced by a Stop button
...
#### Scenario: Stop button aborts WebSocket generation
- **WHEN** the user clicks the Stop button while a WebSocket chat generation is in progress
- **THEN** the composable SHALL send `{ type: "chat:abort", id }` over WebSocket, and upon receiving `{ type: "chat:aborted", id }`, the composable SHALL set `isLoading` to `false`, clear `streamingContent`, and re-enable all input controls

#### Scenario: Stop button aborts HTTP generation
- **WHEN** the user clicks the Stop button while an HTTP chat request is in progress
- **THEN** the composable SHALL abort the in-flight `fetch()` request via `AbortController.abort()`, set `isLoading` to `false`, and re-enable all input controls
...
All API calls (WebSocket messages, POST chat, DELETE chapter) SHALL be extracted into a `useChatApi()` composable or equivalent, keeping the `ChatInput.vue` component focused on UI concerns. The composable SHALL accept the WebSocket connection from a shared `useWebSocket` composable and fall back to HTTP with authentication headers from a shared auth composable when WebSocket is unavailable. The composable SHALL expose an `abortCurrentRequest()` method that aborts the active generation via `chat:abort` (WebSocket) or `AbortController.abort()` (HTTP).
...
- **THEN** the composable SHALL route chat send/resend/abort through the WebSocket channel instead of HTTP
...
- **THEN** the composable SHALL use the existing HTTP endpoints for chat send/resend and use `AbortController` for abort

#### Scenario: Abort method exposed by composable
- **WHEN** the ChatInput component's Stop button is clicked
- **THEN** the component SHALL call `useChatApi().abortCurrentRequest()` which handles the abort logic for the active channel (WebSocket or HTTP)
