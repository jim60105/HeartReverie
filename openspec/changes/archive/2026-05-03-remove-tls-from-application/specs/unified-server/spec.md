...
**Reason:** HeartReverie no longer ships an in-application TLS layer. `scripts/serve.sh` becomes a thin `deno run` invocation that listens on plain HTTP only; cert generation logic is deleted along with `entrypoint.sh`. Operators terminate TLS at their reverse proxy / ingress controller of choice.

## MODIFIED Requirements

### Requirement: Server initialization

The writer backend SHALL be a Deno application using Hono framework with ESM modules. The server SHALL serve the `reader/` directory (or `reader-dist/` for built output) as static files at the root path `/` via Hono's `serveStatic()` middleware. The server SHALL listen on plain HTTP using `Deno.serve()` with no `cert`/`key` options. The server SHALL read the port from the `PORT` environment variable via `Deno.env.get()` or default to `8080`. The server SHALL include an SPA fallback route that serves `index.html` for any GET request not matched by API routes, plugin routes, asset routes, the legacy `/js/` route, or existing static files. This enables Vue Router's HTML5 history mode to work correctly on page refresh and direct URL access.
...
The `scripts/serve.sh` script SHALL start the writer backend via `deno run` with explicit permission flags (`--allow-net`, `--allow-read`, `--allow-write`, `--allow-env`, `--allow-run`). The script SHALL check for the `deno` command instead of `node`. No `node_modules` directory or `package.json` SHALL be required. The script SHALL NOT generate TLS certificates and SHALL NOT delegate to any `entrypoint.sh`.

#### Scenario: Start the unified server

- **WHEN** `scripts/serve.sh` is executed
- **THEN** the script SHALL invoke `deno run` directly with appropriate permission flags, and the server SHALL serve both the static frontend at `/` and the API at `/api/`
...
#### Scenario: No cert generation

- **WHEN** `scripts/serve.sh` is executed
- **THEN** the script SHALL NOT invoke `openssl` and SHALL NOT create a `.certs/` directory or write any certificate or private-key files
