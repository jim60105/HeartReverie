...
#### Scenario: LLM_API_KEY documented as optional
- **WHEN** a contributor inspects the `.env.example` file
- **THEN** the `LLM_API_KEY` entry SHALL be annotated as optional — required only for cloud LLM providers, and omittable for keyless local or private providers (e.g. Ollama, vLLM, LM Studio)
...
- **WHEN** a contributor inspects the `.env.example` file
- **THEN** the `LLM_REASONING_EFFORT` entry SHALL document the default value as `xhigh` and SHALL list the accepted values `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`
