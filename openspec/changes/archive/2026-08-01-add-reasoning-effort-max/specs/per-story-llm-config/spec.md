...
- **WHEN** an authenticated client PUTs `{ "reasoningEffort": "low" }` or `{ "reasoningEffort": "max" }` to the per-story config endpoint
- **THEN** the server SHALL persist `{ "reasoningEffort": "low" }` / `{ "reasoningEffort": "max" }` to `_config.json` and respond HTTP 200 with the persisted object
