...
Defines the structured "cards" UI for the Prompt Editor — a per-message visual editor that parses the `system.md` Vento source into one card per `{{ message "<role>" }}…{{ /message }}` block, lets the user edit each card's role and body independently, and serialises back to a canonical Vento source on save. This capability covers the `MessageCard` data type, the parser, the serialiser, the per-card SFC, the card-list manipulation helpers, the per-card delete-confirmation flow, and the pre-save validity guard that mirrors the backend's `multi-message:no-user-message` / `multi-message:empty-message` rules.
