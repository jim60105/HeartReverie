...
- **THEN** the `state-patches` plugin's hook handler SHALL execute `execFile('./plugins/state-patches/rust/target/release/state-patches', ['playground'])`, await its completion, and the hook chain SHALL continue
...
- **WHEN** the `state-patches` binary does not exist at the expected path `./plugins/state-patches/rust/target/release/state-patches`
...
**Removed**: The direct `execFile` call in `server.js` after AI response completion is replaced by the plugin hook system. The status-bar plugin now owns the `state-patches` invocation via a `post-response` hook handler, enabling the behavior to be enabled/disabled through the plugin system rather than requiring code changes. The hardcoded `execFile('./plugins/state-patches/rust/target/release/state-patches', ['playground'])` call in `server.js` has been removed. The status-bar plugin's `post-response` hook handler provides the same functionality. The `execFile` safety requirements (explicit arguments, no shell, no user input interpolation) are preserved in the plugin handler implementation.
