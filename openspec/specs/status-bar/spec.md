...
- **post-response hook**: The plugin SHALL register a `post-response` hook handler that invokes the `state-patches` binary after each completed AI response (replacing the hardcoded invocation in `server.js`)
...
3. Register a `post-response` hook handler that executes `./plugins/state-patches/rust/target/release/state-patches playground` via `execFile`
...
- **THEN** the plugin SHALL register its manifest with type `full-stack`, register its `<status>` tag handler with the md-renderer, register a `prompt-assembly` handler for `status.md`, and register a `post-response` hook handler for state-patches execution
...
The status-bar plugin SHALL register a `post-response` hook handler that replaces the hardcoded `state-patches` invocation previously in `server.js`. After each completed AI response, the hook system SHALL invoke registered `post-response` handlers in priority order. The status-bar plugin's handler SHALL execute `./plugins/state-patches/rust/target/release/state-patches playground` using `execFile` (not `exec`). The handler SHALL await completion before the hook chain continues. If the command exits with a non-zero status code or writes to stderr, the handler SHALL log a warning but SHALL NOT fail the hook chain or the HTTP response. If the `state-patches` binary is not found, the handler SHALL log a warning and return without error.
...
- **THEN** the status-bar plugin's hook handler SHALL execute `execFile('./plugins/state-patches/rust/target/release/state-patches', ['playground'])` and await its completion
