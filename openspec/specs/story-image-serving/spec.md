...
HTTP endpoints that serve generated story image files and their metadata from the per-story `_images/` directory under `PLAYGROUND_DIR`, with passphrase authentication, strict filename validation, content-type inference, long-lived caching, and `X-Content-Type-Options: nosniff` to defeat MIME-sniffing attacks.
