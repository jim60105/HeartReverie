# Dialogue Colorize Plugin

## Purpose

Built-in frontend-only plugin that visually colourizes dialogue runs (text inside supported quote pairs) in rendered chapters using the CSS Custom Highlight API, without mutating the chapter DOM, the underlying `.md` files, the prompt, or the export output.
