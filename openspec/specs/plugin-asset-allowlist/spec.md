# plugin-asset-allowlist Specification

## Purpose

Defines the HTTP contract that gates the wildcard `GET /plugins/:plugin/:path{.+\.js}` route serving plugin-provided ES modules to the SPA. The capability establishes a manifest-driven per-plugin allowlist (`PluginManager.getPluginAllowedJsFiles(name)`) composed from the validated, normalized union of `manifest.frontendModule` and `manifest.frontendImports`, plus complementary defenses: pre-normalization rejection of paths containing literal backslashes, dotfile segment rejection, raw plugin-directory containment via `String.startsWith(pluginDir + SEP)`, and symlink-safe containment via `Deno.realPath()`. Together these layers ensure only declared, on-disk `.js` assets inside the plugin directory can ever be served.
