# Response Notify Plugin

## Purpose

Frontend-only plugin that emits notifications when LLM story generation completes, leveraging the notification hook stage and the `useNotification` composable to keep users informed regardless of tab visibility.
