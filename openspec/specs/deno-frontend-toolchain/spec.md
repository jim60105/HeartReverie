...
This specification covers the frontend (`reader-src/`) dependency management and build pipeline using Deno. All frontend npm packages are declared in the root `deno.json` using `npm:` specifiers, task commands use `deno run` instead of `npx`, and the `node_modules` directory is eliminated in favor of Deno's global cache.
...
1. `reader-src/package.json` and `reader-src/package-lock.json` do not exist in the repository
2. All 15 frontend npm packages are declared in `deno.json` imports with `npm:` specifiers
3. `deno task dev:reader`, `deno task build:reader`, and `deno task test:frontend` all succeed without `npx` or `npm`
4. `deno.lock` contains locked versions for both backend JSR packages and frontend npm packages
5. `reader-src/vite.config.ts` uses `import.meta.dirname` instead of `__dirname`
6. `reader-src/tsconfig.json` does not exclude `node_modules`
7. No `npm install` step is required to resolve frontend dependencies
8. `nodeModulesDir: "auto"` is set in `deno.json` and root `node_modules/` is gitignored
